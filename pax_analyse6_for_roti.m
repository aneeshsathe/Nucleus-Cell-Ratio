...
root_root_root='/mnt/mbi/images/aneesh/IT Core Work/Nuclear Total Ratio/Eric/Data/';
...
root_fold_list=dir(root_root_root);
for root_fold_count=3:size(root_fold_list,1)
root_root=[root_root_root,root_fold_list(root_fold_count).name];
...
for fold_count=3:size(fold_list,1)
    ...
    [xls_label{1,1:15}]=deal(...
        ...
        'Avg Int Ratio','Cell thresh val','Nuc thresh val','Std in Nuc'...%12-15
        );
    %%
...
%             cell_stack=reshape(imfill(reshape(pro_cell_stack,[],size(pro_cell_stack,1)*size(pro_cell_stack,2))),size(pro_cell_stack));
            ...
            [pro_xls_label{1,1:15,1}]=deal(zeros);
