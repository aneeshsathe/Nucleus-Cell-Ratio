...
root_root='Y:\IT Core Work\Nuclear Total Ratio\Eric\Data\test\';
...
cell_3d_area_thresh= 20000;%spinning disk normal cells
%  cell_3d_area_thresh= 3000;%spinning disk fak-/- or vin-/- (small cells)
nuc_3d_area_thresh=30000;
...
%     file_path=[root, file_list(end).name]; %modification
    ...
    [xls_label{1,1:18}]=deal(...
        'Cell','Time', ... %1-2
        'Nuc Intensity', 'Cell Intensity','Ratio', 'Nuc Vol','Cell Vol',...%3-7
        'Nuc Label Int ','nuc_coloc_ratio','Avg Nuc Int', 'Avg Cell Int',...%8-11
        'Avg Int Ratio','Cell thresh val','Nuc thresh val','Nuc Cyto Ratio',...%12-14
        'Nuc Height','Cell Proj Area','Nuc Proj Area'...
        );
    %% OLD
...
%             temp=bfGetStack(reader,cell_chanl,z_range,t_count-1);
            ...
            [pro_xls_label{1,1:18,1}]=deal(zeros);
            if ~isempty(bw_re_prop)
                ...
                for obj_count=1:size(obj_idx,2)
                    ...
                    %%
                    %get pix list
                    prop_cell_bw=regionprops(cell_bw,'Area', 'PixelIdxList','BoundingBox','FilledImage');
                    prop_nuc_bw=regionprops(nuc_bw,'Area', 'PixelIdxList','BoundingBox','FilledImage');
                    
                    if (sum([prop_cell_bw.Area]>cell_3d_area_thresh)&&sum([prop_nuc_bw.Area]>nuc_3d_area_thresh))&&numel(prop_cell_bw)>1==0
                        %                         disp('passed')
...
%                             nuc_std,cell_proj_area.Area...%16
%                             );
...
%                         max_img=max(BS_orig_cell_stack,[],3);
                        ...
                        %         imtool(out_img,[])
                        %% write image
                        an_write_img_paxanalyse(out_img,max_img,proj_cell_nuc,img_write_path,series_count,obj_count,t_count)
