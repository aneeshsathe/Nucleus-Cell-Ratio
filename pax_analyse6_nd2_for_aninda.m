...
root_root='Y:\IT Core Work\Nuclear Total Ratio\Aninda\test_data\';
...
for fold_count=3:size(fold_list,1)
    ...
    [xls_label{2:size(file_list,1)+1,1:18}]=deal(NaN);
...
%                         figure,imshow(reshape(cell_bw,[],size(cell_bw,2)*size(cell_bw,3)))
                        %%
                        %get pix list
                        ...
                        if (sum([prop_cell_bw.Area]>cell_3d_area_thresh)&&sum([prop_nuc_bw.Area]>nuc_3d_area_thresh))
                            ...
                            clear out_img prop_nuc_bw prop_cell_bw
                    ...
                    xls_label(file_count+1,1:18)=pro_xls_label(:);
